...
"""File for operating on an Azure Fonudry Subclient.

AzureFoundrySubclient is the only class defined in this file.

AzureFoundrySubclient: Derived class from CloudAppsSubclient Base class, representing an
Azure Fonudry Cloud Apps subclient, and to perform operations on that subclient.

AzureFoundrySubclient:

    _get_subclient_properties()         --  Gets the properties of the Azure Fonudry subclient

    _get_subclient_properties_json()    --  Gets the properties JSON of the Azure Fonudry subclient
    ...
    restore()                           --  Restore Azure Fonudry data from the specified backup
...
class AzureFoundrySubclient(CloudAppsSubclient):
    """Represents an Azure Fonudry Cloud Apps subclient.
    ...
    def _build_content_item(content_path: str) -> dict:
        """Wrap a plain Azure Fonudry content path in CloudDBEntity XML format.
    ...
    def _get_subclient_properties(self) -> None:
        """Retrieve the properties specific to the Azure Fonudry subclient.

        #ai-gen-doc
        """
        super(AzureFoundrySubclient, self)._get_subclient_properties()
        self._azure_foundry_content = self._subclient_properties.get('content', [])

    def _get_subclient_properties_json(self) -> dict:
        """Build the properties JSON payload for updating the Azure Fonudry subclient.

        Returns:
            dict: A dictionary containing all properties of the Azure Fonudry subclient.
        ...
        return {
            'subClientProperties': {
                ...
                'content': self._azure_foundry_content
            }
        }
    ...
    def _set_content(self, items: Optional[List[str]] = None) -> None:
        ...
        Args:
            items: List of plain Azure Fonudry content path strings.

        #ai-gen-doc
        """
        self._azure_foundry_content = [
            self._build_content_item(item) for item in (items or [])
        ]
        self._set_subclient_properties('content', self._azure_foundry_content)
    ...
    def content(self) -> List[str]:
        ...
        #ai-gen-doc
        """
        parsed = [self._parse_content_name(item) for item in self._azure_foundry_content]
    ...
    def browse(self, *args: Any, **kwargs: Any) -> dict:
        """Browse the content of this Azure Fonudry subclient's instance backups.
    ...
    def restore(
        ...
        """Restore Azure Fonudry data from the specified backup.
