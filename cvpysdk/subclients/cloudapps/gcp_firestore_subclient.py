...
"""File for operating on a GCPFirestore Subclient.

GCPFirestoreSubclient is the only class defined in this file.

GCPFirestoreSubclient:  Derived class from CloudAppsSubclient Base class, representing a
                     GCPFirestore Cloud Apps subclient, and to perform operations on that subclient

GCPFirestoreSubclient:

    _get_subclient_properties()         --  Gets the properties of the GCPFirestore subclient

    _get_subclient_properties_json()    --  Gets the properties JSON of the GCPFirestore subclient
    ...
    _build_content_xml()                --  Build a CloudDBEntity XML string from a list of database names
    ...
    restore()                           --  Restore GCPFirestore data from the specified backup
...
class GCPFirestoreSubclient(CloudAppsSubclient):
    """
    Represents a GCPFirestore Cloud Apps subclient for managing backup and restore operations.

    This class extends the CloudAppsSubclient base class and provides specialized methods
    for interacting with GCPFirestore subclients.
    ...
    def _build_content_item(db_name: str) -> dict:
        ...
        Args:
            db_name: Plain GCPFirestore database name, e.g. "MY_DB".
    ...
    def _build_content_xml(db_names: List[str]) -> str:
        ...
        by the GCPFirestore instance creation API.

        Args:
            db_names: List of plain GCPFirestore database name strings.
    ...
    def _get_subclient_properties(self) -> None:
        """Retrieve the properties specific to the GCPFirestore subclient.
        ...
        #ai-gen-doc
        """
        super(GCPFirestoreSubclient, self)._get_subclient_properties()
        self._gcp_firestore_content = self._subclient_properties.get('content', [])

    def _get_subclient_properties_json(self) -> dict:
        """Build the properties JSON payload for updating the GCPFirestore subclient.

        Returns:
            dict: A dictionary containing all properties of the GCPFirestore subclient
        ...
        subclient_json = {
            "subClientProperties": {
                ...
                "content": self._gcp_firestore_content
            }
        }
    ...
    def _set_content(self, db_names: Optional[List[str]] = None) -> None:
        ...
        #ai-gen-doc
        """
        self._gcp_firestore_content = [
            self._build_content_item(name) for name in (db_names or [])
        ]
        self._set_subclient_properties("content", self._gcp_firestore_content)

    @property
    def content(self) -> List[str]:
        """Get the GCPFirestore database names configured as content for this subclient.
        ...
        #ai-gen-doc
        """
        names = [self._parse_db_name(item) for item in self._gcp_firestore_content]
        return [n for n in names if n is not None]

    @content.setter
    def content(self, db_names: List[str]) -> None:
        """Set the databases to back up for this GCPFirestore subclient.

        Args:
            db_names: A non-empty list of plain GCPFirestore database name strings.
    ...
    def browse(self, *args: Any, **kwargs: Any) -> dict:
        """Browse the content of this GCPFirestore subclient's instance backups.
    ...
    def restore(
        ...
        """Restore GCPFirestore data from the specified backup.

        Submits an in-place restore job for the specified paths to restore GCPFirestore
        ...
        Example:
            >>> job = subclient.restore(paths=["/MY_DB"], overwrite=True)
